...
class VirtualNetworkTopology(Topo): 
	...
	def _link_routers_best_path(self, routers: list[Router], dijkstra_reverse_graph: dict[Router, RouterPathNode | None], virtual_network: VirtualNetwork):
	...
	def _propagate_routes(self, routers: list[Router], virtual_network: VirtualNetwork):
		...
		# Analyse all routers in the topology
		for src_router in routers:
			
			# Get the virtual router object
			src_virtual_router = _get_virtual_router(src_router)
    
			# Analyse all objects connected to this router!
			for link in src_router.get_links():
				# Ensure that the endpoint of the link is a router (as we need to update the routing table)
				dst_router = link.endpoint.entity
				if not isinstance(dst_router, Router):
					continue

				# Get the virtual router object for both routers
				dst_virt_router = _get_virtual_router(dst_router)

				# Propagate the routes reachable from the source router to the destination router
				# We need to register only the routes that are not already present in their routing table
				
				# Get links of the source router
