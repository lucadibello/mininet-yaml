...
def traffic_engineering_task_from_virtual_network(topology: NetworkTopology, virtual_network: VirtualNetwork) -> Tuple[CBCMIPSolver, TrafficEngineeringLPResult]:
    ...
    for demand in topology.get_demands():
        ...
        counter += 1
    # Append flows to task
    task.set_flows(flows)
    ...
    for demand, flow_name in flows.items():
        ...
        variable_lookup[SRC_OVERALL_FLOW_NAME + "_" + flow_name] = glop.solver.IntVar(0, demand.maximumTransmissionRate, SRC_OVERALL_FLOW_NAME + "_" + flow_name)
