...
class LPNetwork():
    ...
    def find_paths(self, src: VirtualNetworkElement, dst: VirtualNetworkElement) -> list[list[VirtualNetworkElement]]:
        # Validate input
        ...
        # Run DFS to find all possible paths from src to dst
        found_paths = list[list[VirtualNetworkElement]]()
        visited = set[VirtualNetworkElement]()
        stack: list[tuple[VirtualNetworkElement, list[VirtualNetworkElement]]]= [(src, [])]
        while stack:
            # Get next element to visit
            ...
            # If we have not reached the destination, keep exploring
            for neighbor in self._adjacency_list[current]:
                # If we have not visited the neighbor, add it to the stack
                
                # FIXME: I NEED TO REGISTER ALSO THE USED ROUTE. IN THIS CASE EVERY ELEMENT CAN BE VISITED ONLY ONE TIME
                if neighbor not in visited:
                    print(f"Adding {neighbor.get_name()} to the stack")
                    stack.append((neighbor, path + [neighbor]))
    
        # We return all the paths
        return found_paths
